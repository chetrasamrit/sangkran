<template>
  <div class="bg-red-100">
    ...
    <div class="py-10 px-4 text-center">
      <h2 class="text-xl md:text-xl font-bold text-gray-800">
    ...
    <div>
      ...
      <router-link v-for="news in homeState.newsItems" :key="news.id" :to="`/events/${news.id}`"
        ...
        <div class="p-4">
          ...
          <p class="text-gray-700">{{ news.description.slice(0, 100) }}{{ news.description.length > 100 ? '...' : '' }}</p>
        </div>
      </router-link>
    </div>
    ...
    <div class="relative">
      <img :src="sk2" class="w-full h-auto object-cover" />
      <h2 class="text-center text-xl md:text-2xl font-bold py-6">{{homeState.sangkran1}}
        {{ homeState.helperMessage }}
      </h2>
    </div>
...
import HeroSecteion from '../components/home/HeroSecteion.vue'
